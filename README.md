...
- **색상**: 파스텔 핑크 (#ff9b9b) + 화이트 + 다크그레이
- **레이아웃**: 카드 기반, 넓은 여백, 큰 border-radius
- **애니메이션**: Framer Motion으로 부드러운 전환 효과
- **타이포그래피**: 명확한 계층 구조, 가독성 우선
...
- 하드코딩된 값 사용 금지
- 공통 컴포넌트 재사용으로 일관성 유지

상세한 스타일링 규칙은 [STYLING_RULES.md](./STYLING_RULES.md)를 참고하세요.
...
- **대안 고려**: Redux는 오버 엔지니어링 판단
- **최적화**: products는 constants에서 가져오고, change는 파생 상태로 처리
...
- **이유**: CSS-in-JS로 컴포넌트와 스타일 강한 결합
- **Theme 시스템**: 모든 색상/간격/폰트 중앙 관리
- **일관성**: 공통 컴포넌트 재사용으로 중복 최소화
